...
public class SourceRedisArgs implements RedisArgs {
    ...
    @Option(names = "--source-cluster", defaultValue = "${RIOT_SOURCE_CLUSTER}", description = "Enable source cluster mode.")
    private boolean cluster;

    @Option(names = "--source-resp", defaultValue = "${RIOT_SOURCE_RESP:-RESP2}", description = "Redis protocol version used to connect to source: ${COMPLETION-CANDIDATES} (default: ${DEFAULT-VALUE}).", paramLabel = "<ver>")
    private ProtocolVersion protocolVersion = DEFAULT_PROTOCOL_VERSION;

    @Option(names = "--source-pool", defaultValue = "${RIOT_SOURCE_POOL:-8}", description = "Max number of source Redis connections (default: ${DEFAULT-VALUE}).", paramLabel = "<int>")
    private int poolSize = DEFAULT_POOL_SIZE;

    @Option(names = "--source-read-from", defaultValue = "${RIOT_SOURCE_READ_FROM:-UPSTREAM}", description = "Which source Redis cluster nodes to read from: ${COMPLETION-CANDIDATES} (default: ${DEFAULT-VALUE}).", paramLabel = "<n>")
