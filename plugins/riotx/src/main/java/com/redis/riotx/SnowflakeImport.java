...
public class SnowflakeImport extends DatabaseImport {
    ...
    // reuse the sql param
    // TODO find a way to override the description or method on that param or maybe this shouldnt extend DatabaseImport
    // @Option(names = "--table", required = true, description = "Fully qualified table: ex MYDATABASE.MYSCHEMA.TABLE", paramLabel = "<table>")
    // private String table;

    @Option(names = "--start-from-beginning", description = "Create the snowflake stream with SHOW_INITIAL_ROWS=TRUE to import initial data + changes from that point.", paramLabel = "<startFromBeginning>")
    ...
    private String getCurrentOffset(Connection sqlConnection, String fullStreamName) throws SQLException {
        String offsetSql = MessageFormat.format("SELECT SYSTEM$STREAM_GET_TABLE_TIMESTAMP(''{0}'')",
                fullStreamName);
        ...
        try {
            ...
            if (results.next()) {
                ...
                throw new RuntimeException("getCurrentOffset query did not return any results. This should not happen: " + offsetSql);
            }
    ...
    private Runnable afterSuccess(Connection sqlConnection, RedisContext redisContext, String fullStreamName, String offsetKey, String tempTable){
        ...
        return () -> {
            ...
            try {
                ...
                log.debug("in snowflake import after success: stored offset {}={}",
                        offsetKey, newOffset);

                // TODO cleanup but ensure this doesn't interfere with any other tasks or jobs
                // sqlConnection.prepareStatement(MessageFormat.format("DROP TABLE {0}", tempTable)).execute();
    ...
    private String initStatement(String currentOffset, String fullStreamName) {
        ...
        if (currentOffset != null) {
            initStatement = MessageFormat.format("CREATE OR REPLACE STREAM {0} ON TABLE {1} AT (TIMESTAMP => TO_TIMESTAMP(''{2}''))",
                    fullStreamName, table, currentOffset);
        } else {
            initStatement = MessageFormat.format("CREATE OR REPLACE STREAM {0} ON TABLE {1}",
                    fullStreamName, table);
            if (fromBeginning) {
    ...
    protected void initialize() throws RiotInitializationException {
        ...
        fullStreamName = MessageFormat.format("{0}.{1}.{2}",
                database, schema, streamName);
    ...
    protected JdbcCursorItemReader<Map<String, Object>> reader() {
        ...
        itemReader.setPreparedStatementSetter(ps -> {
            try {
                Connection dbConnection = getDataSourceArgs().dataSource().getConnection();
                ...
                if (null != newOffset){
                    syncCommands.set(offsetKey, newOffset);
                    log.debug("in snowflake reader setup: stored offset {}={}",
                            offsetKey, newOffset);
                }

                String initStatement = initStatement(currentOffset, fullStreamName);

                // TODO figure out why we cant use temp table here - could be multiple differenct connection/sessions use it
                String initTempTable = MessageFormat.format("CREATE OR REPLACE TABLE {0} AS SELECT * FROM {1}",
                        tempTable, fullStreamName);
                ...
                throw new RuntimeException("Unable to execute pre sql for snowflake-import", e);
            }
        });
        return itemReader;
    }
