...
[source,console,subs="verbatim,attributes"]
----
{app} faker [OPTIONS] EXPRESSION... [REDIS COMMAND...]
----
...
[IMPORTANT]
====
Redis connection options apply to the root command (`{app}`) and not to subcommands.

In this example the Redis options will not be taken into account:

[source,subs="verbatim,attributes"]
----
{app} faker id="numerify '####'" hset -h myredis.com -p 6380
----
====

TIP: The `_index` field is a monotonous sequence and is available in processors. You can for example do this: `{app} faker text="lorem.words 2" --proc id="_index" hset test:#{id}"
...
[source,console,subs="verbatim,attributes"]
----
{app} faker firstName="name.first_name"
----
...
[source,console,subs="verbatim,attributes"]
----
{app} faker lease="number.digits '2'"
----
