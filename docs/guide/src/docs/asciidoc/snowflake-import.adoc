...
RIOT-X can be deployed on Kubernetes for production workloads using the provided pod configuration.

=== Prerequisites

Before deploying, ensure you have:

* A Kubernetes cluster with access to create pods and secrets
...
The Kubernetes pod configuration uses environment variables following the `RIOT_REDIS_*` and `RIOT_*` naming conventions.

Download the pod configuration:
[source,console,subs="verbatim,attributes"]
----
curl -O {site-url}/snowflake-import-pod.yaml
----

Or use the local file: link:{site-url}/snowflake-import-pod.yaml[snowflake-import-pod.yaml]

=== Secrets Setup

Before deploying the pod, create the required secrets with your actual credentials:
...
Edit the pod YAML to customize for your environment:

[source,yaml]
----
# Update Redis connection
- name: RIOT_REDIS_URI
  value: "your-redis-uri"  # <1>
...
[source,console]
----
kubectl apply -f snowflake-import-pod.yaml
----
...
# View logs
kubectl logs -f riotx-snowflake-import

# Check metrics (if enabled)
kubectl port-forward riotx-snowflake-import 8080:8080
...
* **High Availability**: Use Deployments or Jobs instead of bare pods
...
spec:
  ...
  selector:
    matchLabels:
      app: riotx
  template:
    # ... pod spec from snowflake-import-pod.yaml
----
