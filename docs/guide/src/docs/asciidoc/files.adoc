...
----
{app} file-import [OPTIONS] FILE... [REDIS COMMAND...]
----
...
[IMPORTANT]
====
Redis client options apply to the root command (`{app}`) and not to Redis commands.

In this example Redis client options will not be taken into account:

[source,subs="verbatim,attributes"] 
----
{app} file-import my.json hset -h myredis.com -p 6380
----
====
...
[source,console,subs="verbatim,attributes"]
----
{app} file-import my.json hset blah:#{id}
----
...
[source,console,subs="verbatim,attributes"]
----
{app} file-import my.json hset blah:#{id} expire blah:#{id}
----

.Import into hashes `blah:<id>` *and* set TTL *and* add each `id` to a set named `myset`
[source,console,subs="verbatim,attributes"]
----
{app} file-import my.json hset blah:#{id} expire blah:#{id} sadd myset --member #{id}
----
...
{project-title} supports https://parquet.apache.org/docs/overview/[Parquet] files.

.Parquet file import example
[source,console,subs="verbatim,attributes"]
-----
{app} file-import s3://riotx/userdata1.parquet --s3-region us-west-1 hset user:#{id}
-----
...
[source,console,subs="verbatim,attributes"]
----
{app} file-export [OPTIONS] FILE
----

To show the full usage, run:
[source,console,subs="verbatim,attributes"]
----
{app} file-export --help
----
...
[source,console,subs="verbatim,attributes"]
-----
{app} file-export beers.parquet --parquet-field ounces=DOUBLE abv=DOUBLE id=INT32
-----
