...
public abstract class AbstractTargetTestBase extends AbstractTestBase {
	...
	public static final Duration DEFAULT_TTL_TOLERANCE = Duration.ofMinutes(1);
	...
	protected <K, V> StatefulRedisModulesConnection<K, V> targetRedisConnection(RedisCodec<K, V> codec) {
		return RedisModulesUtils.connection(targetRedisClient, codec);
	}
	...
	protected <K, V> RedisItemWriter<K, V, KeyValue<K>> structWriter(AbstractRedisClient client,
	...
	protected <K, V> List<KeyComparison<K>> compare(TestInfo info, RedisCodec<K, V> codec, KeyComparator<K> comparator)
		...
		RedisItemReader<K, V> targetReader = structReader(testInfo(info, "comparisonReader", "target"),
				targetRedisClient, codec);
	...
	protected <K, V> List<KeyComparison<String>> runAndCompare(TestInfo info, RedisItemReader<K, V> reader,
		...
		if (!mismatches.isEmpty()) {
			List<String> sourceKeys = RedisModulesUtils.connection(reader.getClient()).sync().keys("*");
			Collections.sort(sourceKeys);
			log.info("Source keys: {}", sourceKeys);
			List<String> targetKeys = RedisModulesUtils.connection(writer.getClient()).sync().keys("*");
