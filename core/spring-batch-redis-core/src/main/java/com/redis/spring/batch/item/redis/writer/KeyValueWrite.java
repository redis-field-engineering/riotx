...
public class KeyValueWrite<K, V> implements Operation<K, V, KeyValue<K>, Object> {
	...
	@Override
	public List<RedisFuture<Object>> execute(RedisAsyncCommands<K, V> commands, Chunk<? extends KeyValue<K>> items) {
		List<RedisFuture<Object>> futures = new ArrayList<>();
		futures.addAll(delete.execute(commands, new Chunk<>(toDelete(items))));
		futures.addAll(write.execute(commands, new Chunk<>(toWrite(items))));
		futures.addAll(expire.execute(commands, new Chunk<>(toExpire(items))));
		return futures;
	}

	private List<? extends KeyValue<K>> toWrite(Chunk<? extends KeyValue<K>> items) {
